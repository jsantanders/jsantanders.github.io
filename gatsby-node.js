  ...
  return new Promise((resolve, reject) => {
    ...
    `).then(result => {
      ...
      const pages = posts.filter(
        ({ node }) => node.fields.langKey === 'en' && _.get(node, 'frontmatter.layout') === 'page'
      )
      ...
      const defaultLangPosts = posts.filter(
        ({ node }) => node.fields.langKey === 'en' && _.get(node, 'frontmatter.layout') === 'post'
      );
...
exports.onCreateNode = ({ node, actions }) => {
  ...
  if (_.get(node, 'internal.type') === `MarkdownRemark`) {
    ...
    if (node.frontmatter.tags && node.internal.fieldOwners.slug) {
